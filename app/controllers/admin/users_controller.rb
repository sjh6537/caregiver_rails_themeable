  ...
  class UsersController < ApplicationAdminController
    ...
    def create
      ...
      respond_to do |format|
        if @user.save
          format.html do
            redirect_to admin_users_path, notice: I18n.t('Notify.Note.Account_Created', name: @user.line_name.to_s)
          end
        else
          format.html do
            render action: 'new', alert: I18n.t('Notify.Note.Account_Created_Fail', name: @user.line_name.to_s)
          end
        end
      end
    end
    ...
    def block
      respond_to do |format|
        if @user.update(enable: false)
          ...
          format.html do
            redirect_to admin_users_path, notice: I18n.t('Notify.Note.Account_Blocked', name: @user.line_name.to_s)
          end
        else
          format.html do
            redirect_back fallback_location: '/',
                          alert: I18n.t('Notify.Note.Account_Blocked_Fail',
                                        name: @user.line_name.to_s)
          end
        end
      end
    end
    ...
    def send_message
      if params[:text].present? && params[:msgtype].present?
        ...
        if !params[:id].present? || (params[:id] == 'null')
          ...
          render json: { status: 'success', message: I18n.t('Notify.Note.Line_send_person_success', name: username.to_s) },
                 status: :ok
        end
        return
      end
