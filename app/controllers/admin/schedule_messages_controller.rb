module Admin
  class ScheduleMessagesController < ApplicationAdminController
    include LineHelper
    ...
    def create
      ...
      respond_to do |format|
        if (result = true)
          ...
          format.html do
            redirect_to admin_schedule_messages_path,
                        notice: I18n.t('Notify.Note.Schedule_Message_Created', name: @admin.name.to_s,
                                                                               title: title.to_s)
          end
        else
          format.html do
            redirect_to admin_schedule_messages_path,
                        notice: I18n.t('Notify.Note.Schedule_Message_Created_Fail', name: @admin.name.to_s)
          end
        end
      end
    end
    ...
    def create_job(user_ids, title, text, time)
      @schedule = ScheduleMessage.new(schedule_name: title, user_ids: user_ids, scheduled_time: time,
                                      message_text: text)
