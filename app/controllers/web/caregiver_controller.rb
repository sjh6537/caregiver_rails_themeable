class Web::CaregiverController < ApplicationWebController
      ...
      if @cared.id == @user.id
        ...
        format.html do
          redirect_to web_accept_edit_path,
                      notice: I18n.t('Notify.Note.You_accept_this_caregiver', name: "#{@cared.name}")
        end
      else
        format.html { redirect_to web_accept_edit_path, notice: I18n.t('Notify.Note.Something_Wrong') }
      end
  ...
  def cared_delete
    ...
    redirect_to web_user_careds_path, notice: I18n.t('Notify.Note.You_remove_this_cared', default: '您已成功移除此被照護者')
  end
