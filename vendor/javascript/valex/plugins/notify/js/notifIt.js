...
(function(root, factory) {
        ...
        function _show(){
            if($confirm){
                if(settings.fullscreen){
                    ...
                    $('body').append($confirm)
                    console.log($confirm.outerWidth())
                    $confirm.css({
                        'top': '20px',
                        'left': $(window).width() / 2 - ($confirm.outerWidth() / 2),
                        'display': 'none'
                    })
    ...
    function notif_prompt(config){
        ...
        function _create(){
            ...
            var $cancelButton = $('<button>', {
                'class': 'notifit_prompt_cancel mg-l-20',
                'text': settings.textcancel
            })
            var $message = $('<p>', {
            ...
            var $input_label = $('<label>', {
                'class': 'notifit_prompt_label',
                'text': settings.input_label
            })
            var $input = $('<input>', {
                'type': 'text',
                'class': 'notifit_prompt_input mg-b-10',
                'value': settings.input_default
            })
            var $select_label = $('<label>', {
                'class': 'notifit_prompt_label',
                'text': settings.select_label
            })
            var $select = $('<select>', {
                'class': 'notifit_prompt_select mg-b-10 '
            })
            $prompt = $('<div>', {
                'class': 'notifit_prompt'
            })
            $select.empty();
            $select.append(settings.select_option);
            $select.val(settings.select_default);
